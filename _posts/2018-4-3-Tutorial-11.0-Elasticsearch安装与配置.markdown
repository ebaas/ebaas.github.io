...
{% include note.html content="如果双击elasticsearch.bat不能启动ES，可以用CMD执行运行这个文件来查看具体错误。通常问题是Java8没有安装正确或者是电脑的内存不够运行ES默认的配置（ES默认需要1个G的内存作为Heap空间）。如果是内存不足原因，可以修改ES的config目录下的jvm.options中的Xms和Xmx配置。" %}
